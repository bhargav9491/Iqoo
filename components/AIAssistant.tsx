...
export default function AIAssistant() {
  ...
  const getResponse = (q: string) => {
    ...
    if (lower.includes('what failed')) {
      return `Failed attempt: ${memory.failedAttempt}. Reason: ${memory.failedReason || 'Unknown'}.`;
    }
    if (lower.includes('what should i do') || lower.includes('next')) {
      return `The next recommended action is to: ${memory.recommendedAction || memory.nextStep}.`;
    }
    if (lower.includes('show me') && lower.includes('handover')) {
            ...
            <div className="flex-1 p-4 overflow-y-auto bg-background flex flex-col gap-4">
              {messages.length === 0 && (
                <div className="text-center text-textMuted mt-10 text-sm">
                  ...
                  <p>Ask me about decisions, context, or what to do next in the {workspace} workspace.</p>
                </div>
              )}
              ...
              {messages.map((msg, i) => (
                <div key={i} className={clsx(
                  ...
                  {msg.role === 'ai' && <div className="text-[10px] font-bold text-primary mb-1 uppercase tracking-wider">AUREVEX</div>}
                  {msg.text}
                </div>
              ))}
              ...
              {isTyping && (
                <div className="bg-primary/10 border border-primary/20 self-start text-white rounded rounded-tl-none p-3 flex items-center gap-2">
                  ...
                  <span className="text-xs text-primary">Analyzing work memory...</span>
                </div>
              )}
            </div>
